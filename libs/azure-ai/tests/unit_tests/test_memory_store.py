...
class TestAzureAIMemoryStoreInit:
    ...
    def test_init_with_endpoint_and_credential(self) -> None:
        """Constructor builds AIProjectClient from endpoint+credential."""
        mock_cred = MagicMock()
    ...
    def test_init_is_experimental(self) -> None:
        """AzureAIMemoryStore is marked as @experimental."""
        from langchain_azure_ai._api.base import ExperimentalWarning, is_experimental

        assert is_experimental(AzureAIMemoryStore)
        with pytest.warns(ExperimentalWarning):
            import azure.ai.projects as _azmod

            with patch.object(_azmod, "AIProjectClient", return_value=MagicMock()):
                AzureAIMemoryStore(
                    endpoint="https://example.azure.com/api/projects/proj",
    ...
    def test_init_raises_without_endpoint(self) -> None:
        """ValueError is raised when no endpoint and no env var is set."""
    ...
    def test_init_raises_without_sdk(self) -> None:
...
class TestBatchGetOp:
    ...
    def test_get_always_calls_search_memories(
        self, store: AzureAIMemoryStore, mock_client: MagicMock
    ) -> None:
        """GetOp always queries the Azure AI service."""
        store.batch([GetOp(namespace=("ns",), key="k")])
