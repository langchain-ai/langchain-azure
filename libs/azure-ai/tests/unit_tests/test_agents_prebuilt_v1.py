...
# ---------------------------------------------------------------------------
# Tests for langgraph version guard in V1 create_prompt_agent
# ---------------------------------------------------------------------------


class TestLanggraphVersionGuard:
    """Verify V1 create_prompt_agent raises a clear ImportError on langgraph<1.1.0."""
    ...
    def test_old_langgraph_version_raises_import_error(self) -> None:
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setattr(importlib.metadata, "version", lambda _: "1.0.9")
    ...
    def test_old_langgraph_version_error_includes_installed_version(self) -> None:
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setattr(importlib.metadata, "version", lambda _: "0.2.45")
    ...
    def test_minimum_accepted_version_does_not_raise(self) -> None:
        ...
        mock_client.create.return_value = mock_agent_version

        with pytest.MonkeyPatch().context() as mp:
            mp.setattr(importlib.metadata, "version", lambda _: "1.1.0")
