...
# ---------------------------------------------------------------------------
# Tests for _get_input_from_state
# ---------------------------------------------------------------------------


class TestGetInputFromState:
    """Tests for the _get_input_from_state helper."""

    def test_returns_last_message(self) -> None:
        """Returns the last message in the messages list."""
        from langchain_azure_ai.agents._v2.base import _get_input_from_state

        msg1 = HumanMessage(content="first")
        msg2 = HumanMessage(content="second")
        state = {"messages": [msg1, msg2]}
        assert _get_input_from_state(state) is msg2  # type: ignore[arg-type]
    ...
    def test_raises_when_messages_key_missing(self) -> None:
        ...
        with pytest.raises(ValueError, match="messages"):
            _get_input_from_state({"other_key": "value"})  # type: ignore[arg-type]
    ...
    def test_raises_clear_error_when_messages_empty(self) -> None:
        ...
        with pytest.raises(ValueError, match="at least one message"):
            _get_input_from_state({"messages": []})  # type: ignore[arg-type]
