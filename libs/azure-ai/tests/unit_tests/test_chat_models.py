...
def test_with_structured_output_json_mode(test_llm_json: AzureAIChatCompletionsModel) -> None:
    """Tests with_structured_output using method='json_mode'."""
    # The schema is not actually used by the model in json_mode, but for completeness, pass a dict.
    ...
    messages = [
        SystemMessage(
            content="You are a helpful assistant. When you are asked if this is a test, reply with a JSON object with key 'message'."
        ),
        HumanMessage(content="Is this a test?"),
    ]
