...
"""End-to-end tests for ``AzureAIResponsesAgentHost`` via Starlette TestClient."""
...
def _client(server: AzureAIResponsesAgentHost) -> TestClient:
...
def test_non_streaming_request_returns_completed_response() -> None:
    server = AzureAIResponsesAgentHost(make_echo_graph())
...
def test_streaming_request_emits_sse_lifecycle_events() -> None:
    server = AzureAIResponsesAgentHost(make_streaming_graph())
    with _client(server) as client:
        resp = client.post(
            "/responses",
...
def test_constructor_rejects_non_messages_state_schema() -> None:
    with pytest.raises(ValueError, match="messages"):
        AzureAIResponsesAgentHost(make_custom_state_graph())
