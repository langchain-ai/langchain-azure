...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="NAME=VALUE",
        help=(
            "server option; repeat for multiple options "
...
def _run_server(
    ...
    *,
    ...
    if protocol == "invocations":
        ...
        InvocationsHostServer(graph, options=options).run(host=host, port=port)
        return
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    try:
        ...
        options = _server_options(args.option) if args.option else None
