...
class PromptBasedAgentNode(RunnableCallable):
    ...
    def __init__(
        ...
        Args:
            ...
            model: The model deployment name. Required when creating a new
                agent (when neither agent_name nor agent_id is provided).
                When reusing an existing agent, overrides the agent's model
                if a new version is created.
            instructions: System instructions for the agent. Required when
                creating a new agent. When reusing an existing agent,
                overrides the agent's instructions if a new version is
                created.
            ...
            agent_id: If provided, must be a ``<name>:<version>`` string
                that identifies an existing agent version.  The agent is
                retrieved and reused.  If override parameters (model,
                instructions, etc.) are also supplied, a **new version**
                is created with those values.  Cannot be used together
                with ``agent_name``.
        ...
        if agent_name is not None and agent_id is not None:
            raise ValueError(
                "Cannot specify both 'agent_name' and 'agent_id'. "
                "Use 'agent_id' (<name>:<version>) to pin a specific version, "
                "or 'agent_name' to use the latest version."
            )
        ...
        if agent_id is not None:
            # Parse the agent_id as <name>:<version>
            if ":" not in agent_id:
                raise ValueError(
                    f"agent_id must be in '<name>:<version>' format, "
                    f"got: {agent_id!r}"
                )
            parsed_name, parsed_version = agent_id.split(":", 1)

            try:
                versions = self._client.agents.get(
                    agent_name=parsed_name
                ).versions
                existing = versions.get(parsed_version)
                if existing is None:
                    raise ValueError(
                        f"Version '{parsed_version}' not found for agent "
                        f"'{parsed_name}'. Available versions: "
                ...
                raise ValueError(
                    f"Could not find agent with name '{parsed_name}' in the "
                    "connected project."
                ) from e

            # Check if any override parameters were provided.
            has_overrides = any([
                model is not None,
            ...
            if has_overrides:
                # Create a new version with the overrides
                override_def_params: Dict[str, Any] = {}
                if model is not None:
                    override_def_params["model"] = model
                if instructions is not None:
                    override_def_params["instructions"] = instructions
                if temperature is not None:
                    override_def_params["temperature"] = temperature
                if top_p is not None:
                    override_def_params["top_p"] = top_p

                if tools is not None:
                    tool_defs = _get_v2_tool_definitions(list(tools))
                    override_def_params["tools"] = tool_defs

                definition = PromptAgentDefinition(**override_def_params)
                ...
                logger.info(
                    "Created new version for agent %s: %s (version=%s)",
                    parsed_name,
                    self._agent.id,
                    self._agent.version,
                )
