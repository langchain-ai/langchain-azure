...
from azure.ai.agents.models import Agent, ThreadMessage, ThreadRun
...
class AzureAIAgentsService(BaseLLM):
    ...
    It uses the Azure AI Projects SDK and Azure AI Agents SDK (using an endpoint or connection strings.)
    ...
    def _create_client(self) -> AIProjectClient:
        ...
        This method handles client creation with proper authentication and configuration.
        ...
        if self.endpoint:
            ...
            raise NotImplementedError(
                "Please provide an endpoint or connetcion string."
            )
        ...
        try:
            ...
            for msg in messages:
                if msg.role == "assistant":
                    # msg.content is a list of content objects, extract text from text objects
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        generations = []
        for prompt in prompts:
            generation = await self._agenerate_single(prompt, stop, run_manager, **kwargs)
            generations.append([generation])
    ...
    def create_agent(self, **kwargs: Any) -> Agent:
        """Create a new agent with custom parameters, replacing the current cached agent.
        ...
        Example:
            ...
            agent = service.create_agent(
                ...
                instructions="You are a specialized data analyst focusing on financial data.",
                temperature=0.3
            )
    ...
    def delete_agent(self, agent_id: Optional[str] = None) -> None:
        ...
        if agent_id is None:
            if self._agent is None:
                raise ValueError("No agent to delete. Create an agent first or provide agent_id.")
