...
def _get_client_kwargs(credential: SDK_CREDENTIAL_TYPE = None) -> dict:
    return {"credential": credential, "connection_data_block_size": 256 * 1024}


def _lazy_load_documents_from_blob(blob_client: BlobClient) -> Iterator[Document]:
...
class AzureBlobStorageLoader(BaseLoader):
    """Document loader for loading LangChain Document objects from a container."""
    ...
    def __init__(
        ...
        - blob_names: Optional[Union[str, Iterable[str]]]
            List of blob names to load. If None, no blobs will be loaded
        - prefix: str
            Prefix to filter blobs in the container
        - credential: Optional[Union[AzureSasCredential, TokenCredential]]
            Credential to authenticate with the Azure Storage account.
            If None, DefaultAzureCredential will be used
        - loader_factory: Optional[Callable[[str], BaseLoader]]
            A callable that returns a custom document loader to use for parsing blobs
            downloaded. Ex: PyPDFLoader, CSVLoader, etc.
        """
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        if isinstance(
            self._credential, azure.core.credentials_async.AsyncTokenCredential
        ):
        ...
        container_client = ContainerClient.from_container_url(
            container_url,
            **_get_client_kwargs(self._credential),
        )
        blobs_with_prefix = list(
            container_client.list_blob_names(name_starts_with=self._prefix)
        )
        if self._blob_names:
            for blob_name in self._blob_names:
                if blob_name in blobs_with_prefix:
                    blob_client = container_client.get_blob_client(blob_name)
                    yield from _lazy_load_documents_from_blob(blob_client)
