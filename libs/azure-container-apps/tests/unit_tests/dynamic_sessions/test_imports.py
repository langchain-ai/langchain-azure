...
from langchain_azure_container_apps.dynamic_sessions import __all__
...
def test_main_package_imports_without_deepagents(
    ...
    for key in list(sys.modules):
        if "langchain_azure_container_apps.dynamic_sessions.backends" in key:
            monkeypatch.delitem(sys.modules, key)

    # The main __init__ must succeed: it only imports from tools.sessions
    import langchain_azure_container_apps.dynamic_sessions as pkg
