...
# importorskip, not a plain import: `python-dotenv` only comes with the
# `test_integration` group, and a whole-tree run without it must skip at
# collection rather than error.
dotenv = pytest.importorskip("dotenv")
dotenv.load_dotenv()

from langchain_azure_container_apps.dynamic_sessions import (  # noqa: E402
...
# Release CI runs `make integration_tests` with no Azure resources provisioned,
# so this must skip rather than fail when unconfigured. The data file is read
# inside the test for the same reason: at module scope it would break collection,
# not just the test.
pytestmark = pytest.mark.skipif(
    not POOL_MANAGEMENT_ENDPOINT,
    reason="AZURE_DYNAMIC_SESSIONS_POOL_MANAGEMENT_ENDPOINT is not set",
)


def test_end_to_end(request: pytest.FixtureRequest) -> None:
    ...
    # Synchronous delete, not `close()`: the daemon-thread delete can be
    # killed at interpreter exit, leaking the session until pool cooldown.
    request.addfinalizer(lambda: tool._delete_session_sync(tool.session_id))  # noqa: SLF001
