"""Azure DocumentDB (with MongoDB compatibility) Vector Store API."""
...
from langchain_azure_documentdb._utils import maximal_marginal_relevance
...
class AzureDocumentDBSimilarityType(str, Enum):
...
class AzureDocumentDBVectorSearchType(str, Enum):
...
class AzureDocumentDBVectorSearchCompression(str, Enum):
...
class AzureDocumentDBVectorSearch(VectorStore):
    ...
    Example:
        . code-block:: python
            ...
            vectorstore = AzureDocumentDBVectorSearch(
                collection=entra_authenticated_collection,
                embedding=embeddings,
            )

        See the package README for a ``DefaultAzureCredential`` and PyMongo
        OIDC example.
    """
    ...
    def create_index(
        ...
        if kind == AzureDocumentDBVectorSearchType.VECTOR_IVF:
        ...
        elif kind == AzureDocumentDBVectorSearchType.VECTOR_HNSW:
        ...
        elif kind == AzureDocumentDBVectorSearchType.VECTOR_DISKANN:
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        _metadatas = metadatas if metadatas is not None else [{} for _ in texts_list]
        texts_batch = []
        metadatas_batch = []
        result_ids = []
        for i, (text, metadata) in enumerate(zip(texts_list, _metadatas)):
    ...
    def _similarity_search_with_score(
        ...
        k: int = 4,
        kind: AzureDocumentDBVectorSearchType = AzureDocumentDBVectorSearchType.VECTOR_IVF,
