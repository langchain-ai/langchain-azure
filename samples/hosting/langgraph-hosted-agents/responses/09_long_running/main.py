...
"""Long-running LangGraph workflow (Responses protocol).

Hosts the same multi-agent slogan workflow as the regular workflow sample, but
enables ``resilient_background`` so callers can submit it with
``background=true`` and poll the Responses API while model work is running.
"""
...
class State(TypedDict):
    ...
    ``messages`` is the channel the Responses host reads. Only the formatter
    writes to it, so only its output is returned. ``draft`` is a private
    scratchpad passed between writer, legal reviewer, and formatter.
    """
...
def main() -> None:
    """Start the long-running Responses host."""
    """Start the long-running Responses host."""
    graph = _build_graph(_build_chat_model())

    port = int(os.environ.get("PORT", "8088"))
    ResponsesHostServer(graph, resilient_background=True).run(port=port)
