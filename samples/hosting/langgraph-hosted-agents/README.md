...
| 5 | [Workflows](responses/05_workflows/) | Hand-built multi-node `StateGraph` (plan -> tools -> synthesize) hosted as the Responses API. |
...
| 9 | [Long-running](responses/09_long_running/) | Background Responses API workflow with `resilient_background=True` so long-running graph execution is pinned by the AgentServer Task API lease and can be polled asynchronously. |
| 10 | [Steering](responses/10_steering/) | Steerable background conversation where a newer turn queues behind the active graph run and the active run observes the hosting cancellation signal. |
| 11 | [Recovery](responses/11_recovery/) | Server-side recovery shape for background Responses: the task is re-entered after a crash/restart and the graph uses a durable watermark to avoid repeating committed work. |
