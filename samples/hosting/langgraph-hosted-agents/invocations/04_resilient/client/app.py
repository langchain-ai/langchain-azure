"""Textual interface for a durable Invocations conversation."""
...
class InvocationsCuiApp(App[None]):
    """Interactive full-screen client for one Invocations session."""

    TITLE = "Durable Invocations"
    SUB_TITLE = "LangGraph tool agent"
    MESSAGE_SEPARATOR = "----------------------------------------"
    BINDINGS = [
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="workspace"):
            yield Static(f"Session: {self._conversation.session_id}", id="status")
            with Vertical(id="conversation-area"):
                yield WrappingLog(id="transcript")
                with Horizontal(id="approval-actions"):
                    yield Static(
                        "Sensitive tool call requires approval",
                        id="approval-label",
                    )
    ...
    async def cancel_from_button(self) -> None:
        await self.action_cancel_invocation()

    @on(Button.Pressed, "#approve")
    def approve_from_button(self) -> None:
        self._submit_approval(approve=True)

    @on(Button.Pressed, "#deny")
    def deny_from_button(self) -> None:
        self._submit_approval(approve=False)
    ...
    def render_conversation_change(self, message: ConversationChanged) -> None:
        ...
        if protocol_event is not None:
            self.query_one("#raw-events", WrappingLog).write(
                f"{json.dumps(protocol_event, sort_keys=True)}\n"
            )
            event_type = protocol_event.get("event")
            data = protocol_event.get("data")
    ...
    def _update_current_controls(self, turn: TurnSnapshot) -> None:
        status_text = {
            "sending": "Starting invocation...",
            "polling": "Invocation running in background...",
            "waiting": "Waiting for the hosted agent session...",
            "recovering": "Connection lost; retrying with the same session...",
        ...
        if awaiting_approval:
            ...
            if turn.approval is not None and (
                turn.approval.id != self._focused_approval_id
            ):
                self._focused_approval_id = turn.approval.id
