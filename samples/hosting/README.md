...
| # | File | What it shows |
|---|------|---------------|
| 1 | [sample_01_responses_basic.py](sample_01_responses_basic.py) | Simplest case: host a `create_agent` graph as the Responses API. |
...
| 7 | [sample_07_responses_toolbox.py](sample_07_responses_toolbox.py) | Combines this hosting package with `langchain_azure_ai.tools.AzureAIProjectToolbox` — tools are loaded at startup from an Azure AI Foundry Toolbox (a managed multi-MCP gateway) and bound to a `create_agent` graph hosted as the Responses API. |
...
| 9 | [sample_09_responses_foundry_memory.py](sample_09_responses_foundry_memory.py) | Foundry's two memory tiers in one hosted graph: `FoundryCheckpointSaver` for short‑term thread state, plus **both** long‑term access patterns against the same Foundry Memory Store — *Pattern B* (a `recall` node that always injects recalled facts as transient context before the LLM) and *Pattern A* (`search_memory` / `save_memory` exposed as `@tool`s the model can call). Two `conversation.id`s for the same `scope` (e.g. `user:demo-user`) share recalled facts. |
